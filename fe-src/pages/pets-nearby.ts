...
export function initPetsNearbyPage() {
  ...
  const grid = pageEl.querySelector("#pets-grid") as HTMLElement;
  const modalContainer = pageEl.querySelector("#modal-avistaje-container") as HTMLElement;
  const modalTitle = pageEl.querySelector("#modal-title") as HTMLElement;
  const formAvistaje = pageEl.querySelector("#form-avistaje") as HTMLFormElement;
  let currentPetIdForAvistaje: number | null = null;

  function renderPets(pets: any[]) {
    ...
    pets.forEach((pet: any) => {
      const card = document.createElement("pet-card-comp") as any;
  ...
  grid.addEventListener("report-avistaje", (e: any) => {
  ...
  async function initLoad() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(async (position) => {
        if (!document.contains(pageEl)) return;
