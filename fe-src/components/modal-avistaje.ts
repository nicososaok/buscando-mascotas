...
export class ModalAvistaje extends HTMLElement {
  ...
  open(petId: number, petName: string) {
    this.currentPetId = petId;
    const overlay = this.querySelector(".modal-overlay") as HTMLElement;
    const title = this.querySelector(".modal-title") as HTMLElement;

    if (title) title.textContent = `Reportar avistaje de ${petName}`;
    if (overlay) overlay.style.display = "flex";
  }

  close() {
    const overlay = this.querySelector(".modal-overlay") as HTMLElement;
    const form = this.querySelector(".form-avistaje") as HTMLFormElement;
